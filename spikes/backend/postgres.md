...
CREATE TABLE expenses (
  ...
  recurring interval NOT NULL CHECK (recurring > interval '0'),
...
- API `recurring` becomes PostgreSQL `interval`; app-level validation should
  enforce RFC 3339 duration compatibility before inserting.
- API Unix millisecond timestamps are represented in Postgres as `timestamptz`
  and converted to/from Unix milliseconds at the API boundary. Plain
  `timestamp` is not UTC milliseconds; it is a date-time without timezone.
