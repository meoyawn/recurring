-- +goose Up
CREATE TABLE users (
    id          text        NOT NULL PRIMARY KEY DEFAULT ('usr_' || encode(gen_random_bytes(8), 'hex')) CHECK (id ~ '^usr_'),
...
CREATE TABLE sessions (
    id         text        PRIMARY KEY DEFAULT ('sess_' || encode(gen_random_bytes(8), 'hex')) CHECK (id ~ '^sess_'),
