...
const mkApp = (): Hono<{ Bindings: EnvVars }> => {
  ...
  app.get(Paths.home, async c => {
    ...
    if (lastProjectID !== undefined) {
      return c.redirect(
        new URL(Paths.project(lastProjectID), c.req.url).toString(),
        302,
      )
    }
    ...
    try {
      ...
      const location = new URL(Paths.project(projectID), c.req.url).toString()
      const secure = isSecureRequest(c.req.raw)
      const headers = new Headers({
        Location: location,
        "Set-Cookie": lastProjectIDCookie(projectID, secure),
      })
  ...
  app.get(Paths.project(":id"), async c => {
    ...
    c.header(
      "Set-Cookie",
      lastProjectIDCookie(projectID, isSecureRequest(c.req.raw)),
    )
